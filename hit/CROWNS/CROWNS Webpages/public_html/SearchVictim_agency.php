...
<td width='50%'><b><a href="manshelter.php">View Shelter Information</a></b></td>
        ...
        <div class="cforms">
          <script>
            function fiend_missing_person(){
              window.location.href = 'locate_agency.php';
            }
            function missing_persons_entry(){
              window.location.href = 'locate_missing_entry_agency.php';
            }
          </script> 
        </div>
        <!-- end section -->
      ...
      <form class = "sky-form" id = "f1" action="" method="post">
        <header> 
          <strong>
            <center>Missing Person's Information
            </center>
          </strong>
        </header>
        <header> 
          <label style="cursor: pointer;">
            <strong>Find missing person
            </strong>
          </label>
          <input type="radio" onClick="fiend_missing_person()" name="gender" id = "resource" value="resour" class="sky-form" />
          <label style="margin-left: 23%; cursor: pointer;">
            <strong>Enter missing person's details
            </strong>
          </label>
...
require("connect.inc.php");?>
  <div class="two_third last">
    <form class = "sky-form">
      <header> 
        <strong>
          <center>List of Missing Persons
          </center>
        </strong>
      </header>
    </form>
...
while ($row=$result_sub->fetch_array()) { ?>
      <tr>
        ...
        <td>
          <h6 style="color:#000080">
            <?php if(empty($row[9])) echo "No Picture Given"; else{
echo "<button class='btn-link' id = 'click'>Click To View Picture</button>" ;?> 
            <script>imgtag = "<?php echo $row[9]; ?>";
            </script>
            <?php }?>
          </h6>
        </td>
...
$set_result = mysql_query("INSERT INTO admin_missing_master VALUES('','$firstname','$lastname','$lastseenat','$phone1','$phone2','','$gender','$age')");
}
else
{
